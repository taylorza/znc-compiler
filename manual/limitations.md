...
- `fixed` (Q4 fixed-point) arithmetic uses 16-bit signed values; overflow behaviour is the same as `int` overflow on the Z80.
- Comments inside `__asm__` blocks (`//`) are stripped by the ZNC scanner and do not appear in the generated assembly output.
