...
typedef enum BASE_TYPE { VOID, CHAR, INT, STRING, STRUCT } BASE_TYPE;
typedef enum MOD_TYPE {CONST=128} MOD_TYPE;

typedef struct  {
    BASE_TYPE basetype;
    int16_t dim; // 0-pointer, 1-scalar, <0-array
    uint16_t struct_id; // non-zero when basetype == STRUCT
} TYPEREC;
...
uint8_t is_struct(const TYPEREC* type) MYCC;
...
void set_struct(TYPEREC* type, uint16_t struct_id) MYCC;
