...
uint16_t parse_string_initializer(uint8_t element_type_id, uint16_t expected_count) MYCC {
    PROLOG(42)
    uint16_t r = far_parse_string_initializer(element_type_id, expected_count);
    EPILOG_RETURN(r)
}
